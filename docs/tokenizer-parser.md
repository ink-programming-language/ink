...
二元运算使用显式栈保存待归约的运算符和左操作数，按优先级与左结合规则构造节点，不为更高优先级的右操作数递归调用二元解析。优先级由具名枚举和符号 `switch` 定义，保留 Ink 文法中比较运算不得连续链接的规则。`atAny` 对当前 token 只做一次查找，完整 token 流中的 trivia 不会为每个候选枚举重复扫描。

错误通过 Core 诊断引擎和显式失败状态报告，源码错误使用准确字节范围。语法恢复通过错误节点、缺失结构的锚点和诊断保留失败位置，并保证继续消费或退出；AST 不复刻错误 token 列表。缺失表达式在分隔符、外围子句关键字或明确的下一条语句起点停止，保留起始 token 供外层解析；前瞻区分函数声明与函数类型、`const` 绑定与 `const ref` / `const ptr`，并保留声明前的 `comptime` 前缀。交互模式将可由追加输入完成的 EOF 错误标为 `Incomplete`；已确定的错误仍为 `Complete`。
