...
> 状态：已确认，反射值与描述符固定版本快照；精确物理布局保持 runtime-private，不由 InkIR v0 冻结；议题 22、23、25、28、34、38、57、65 与 IR v0 补充
> 确认日期：2026-08-08
...
实现必须让动态引用或其关联借用状态同时识别实际布局版本，并固定创建该引用时取得的描述符版本快照。反射操作只能使用该 pinned snapshot 中与对象布局版本兼容的描述符与适配器；不能按名称重解析到最新版本。如果相应版本不能固定、描述符不兼容或已进入不可借用的卸载状态，安全 API 必须抛出版本不兼容异常。

Ink v0 热更新要求 ABI 与布局不变，但已有 `DynamicRef`、描述符 view 和派生借用仍保持原版本 pin，新查询才观察新版本。v0 不提供普通对象状态迁移，也不能把迁移当作解决描述符不兼容的隐式路径；对象迁移协议继续作为第 13 节的未来议题。新旧版本的注册与切换仍遵守议题 17。
...
- 若未来需要公开稳定插件 ABI，`TypeHandle`、布局版本令牌和动态引用应采用何种独立版本化的公开表示；InkIR v0 的私有 runtime 表示不因此冻结；
- 动态值参数数组的正式调用约定；
