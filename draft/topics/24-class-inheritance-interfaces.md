...
- 在满足议题 02 的复制规则时按值传递；
- 通过 `[noncopyable]` 禁止复制；
- 使用 `constructor`、`destructor` 和 `defer` 管理资源。
...
let pixel: Pixel;          // 可以直接位于栈上
let image: Pixel[1024];    // 元素直接内联存储
```
...
let entity: Entity = player; // 编译错误：禁止按值切片
```
