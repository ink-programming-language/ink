...
如果构造装饰器仍遵守议题 16 的恰好一次 continuation 规则，它至多能在构造前后执行同步日志、计数、参数检查、耗时统计或异常转换。

它不能安全表达构造装饰器通常被期待提供的行为：
...
允许跳过或多次 continuation 又会直接改变任务身份、所有权、异常和析构协议。因此 v0 不为少量观测用途引入该复杂机制。
...
异步函数稳定入口继续表示最终实现的任务构造入口；议题 58 的装饰器生命周期块中的 `function.entry` 也使用该入口。它不会先进入用户构造装饰器。
