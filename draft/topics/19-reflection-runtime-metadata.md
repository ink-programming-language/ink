...
> 状态：已确认，动态反射句柄固定版本快照；议题 20—26、28、31—35、37、57、61、63、66、IR v0 与 Parser 议题 15 补充
> 确认日期：2026-08-08
...
- 描述符和成员适配器属于具体模块版本；
- 完全限定名称标识类型、字段和函数；
- 函数适配器与其目标函数入口属于同一个固定版本快照；
- 新描述符、函数实现槽和相关注册项在同一事务中发布；
- 卸载旧版本不会删除新版本的同一逻辑描述符。

每次动态查询都从一次原子的注册表快照中同时取得描述符、适配器入口和对应模块版本 pin。`TypeInfo`、`PropertyInfo`、`FunctionInfo` 以及由它们派生的 view 在有效期内固定该版本，不能在后续操作时按完全限定名称重新解析到更新版本。V1 `FunctionInfo` 始终调用 V1 适配器与 V1 目标；提交 V2 后的新查询取得 V2，已有 V1 句柄仍保持 V1，直到最后一个句柄或派生借用释放后才允许回收 V1。

这些句柄不能是会在模块卸载后悬空的无所有权裸指针。其物理 ABI 可以使用 opaque lease、引用计数、epoch guard 或等价机制，但必须实现上述 pinned snapshot 语义；精确表示留给反射运行时，不改变语言级所有权和借用规则。
...
- 反射描述符、句柄和动态值容器的精确物理布局在 InkIR v0 中有意保持 runtime-private；未来若需要稳定插件 ABI，必须单独版本化；
- 安全动态类型查询由议题 31 规定；
