...
当 `child` 到达 `succeeded`，当前等待恢复前必须解除传播登记。解除以后到达的当前任务请求不能再访问已经结束的登记节点或已销毁的临时任务。
...
current task receives request
    ...
    → child may return, delay, or ignore
    → current task remains suspended until child is final
```

`cancel_on_request` 不直接销毁子任务，也不伪造返回值。子任务返回时，本次等待得到它发布的普通结果。
...
如果 waiter A 收到请求，它会明确向共享的 `child` 发出请求，因此 waiter B 也可能观察到由 `child` 响应请求所造成的普通结果。
...
取消请求可能来自任意线程。传播登记被触发时只能执行有界、线程安全的运行时动作，例如：
...
它不能在请求者线程上同步执行目标任务主体、普通用户回调、可能阻塞的清理或需要报告可恢复问题的代码。用户对请求的响应仍发生在目标任务自己的正常执行路径中。
...
被唤醒的 I/O API 仍按议题 48 自行选择返回普通状态、部分结果或继续操作；语言不加入统一取消结果。

## 12. 与组合等待的关系

库可以定义组合器，在某个普通返回值满足库约定条件后向其他输入任务发布请求。此类组合器与 `cancel_on_request(...)` 共用议题 49 的目标任务请求协议，但触发条件由库 API 显式说明。

核心 `all(...)` 和普通 `await` 都不会自动建立当前任务到输入任务的请求传播。
