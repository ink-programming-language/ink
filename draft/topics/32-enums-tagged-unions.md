...
匹配过程读取判别状态并只进入对应分支，不复制或构造非活动载荷。议题 33 规定模式绑定默认只读借用载荷，显式 `var` 才能取得可变借用；匹配不会隐式消费或移动不可复制载荷。
...
分支切换、枚举赋值以及用户是否可以为枚举声明方法或 `destructor` 的具体语法留给后续议题；任何规则都必须保持“始终只有一个活动分支”的不变量。
...
`Optional<T&>` 仍传播议题 04 的短期引用逃逸限制，不因被包在枚举载荷中而延长生命周期。

`Optional` 的 `if let` 解包和 `match` 规则由议题 33 规定。议题 34 已确定不提供后缀 `?` 传播；便捷方法和额外构造 API 不属于枚举布局本身，留给标准库议题。
