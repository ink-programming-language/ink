...
```ink
let a, b = await all(task_a(), task_b());

let c, d = await all_cancel_on_error(task_c(), task_d());
```
...
```ink
let left, right = await all_cancel_on_error(load_left(), load_right());
```
