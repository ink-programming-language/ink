...
新调用可以进入 V2；已经创建的 V1 任务继续使用 V1 的 resume、destroy、异常描述符和调试信息，直到任务完全销毁后解除模块固定。
...
构造入口使用 LLVM coroutine intrinsics 或 Ink 自有状态机建立帧，并把 resume/destroy 函数指针写入任务控制状态。热更新入口、模块固定和异常边界由 Ink 运行时管理；LLVM 无需理解虚函数与异步任务之间的关系。
