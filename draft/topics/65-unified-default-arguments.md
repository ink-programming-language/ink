...
默认表达式可以产生普通运行时副作用。副作用发生在进入目标函数之前；需要表达可恢复问题时，默认表达式通过普通值或状态把它传给目标函数，不触发隐式控制流。
...
默认实参在进入函数稳定入口和装饰器链之前已经补全。同步装饰器总是看到完整参数列表，不观察默认表达式自身的求值过程。

异步函数调用同样先求值全部显式和默认实参，再调用议题 44、45、48—52、54—56 的任务构造入口：
...
因此默认表达式的副作用发生在任务创建之前，不属于惰性任务体。需要表示可恢复问题的默认表达式必须通过普通值或状态表达；任务只在完整实参准备好之后构造。
