...
普通 `T&`、`const T&`、原始指针和原始切片只要满足结果复制要求，都可以作为任务结果；它们不拥有目标，目标生命周期由程序员负责。安全切片仍使用自己的不逃逸规则，不能作为长期任务结果。
...
议题 69 的元组结果按照元素组合复制能力。包含普通引用的元组可以作为任务结果，任务只保存和复制非拥有别名；程序员必须保证每次 `await` 使用结果时目标仍然有效。任务不会为了异构结果引入隐藏移动或拥有型元组装箱。包含具有独立不逃逸规则的安全切片或其他受限视图时仍须遵守对应类型规则。
