...
> 状态：已确认，议题 27、32、43—45、54、60、61、69 与 Parser 议题 26 补充
> 确认日期：2026-08-01
...
`T&` 与 `const T&` 是可返回、可保存但不拥有目标的 C++ 风格引用。它们可以进入全局、对象字段、闭包、异步任务和其他长期结构，也可以作为普通函数返回类型。

复制引用只复制别名，不复制目标对象。引用不会增加引用计数，不会把目标移动到稳定存储，也不会延长目标生命周期。Ink 不通过生命周期参数、借用检查器或跨函数来源分析证明引用有效。

程序员负责保证每次引用访问时目标地址、生命周期、并发和可变性前置条件成立。目标已经析构、移动或因容器重分配而失效后继续访问引用属于 UB。编译器可以诊断明显悬空情况，但不提供内存安全保证。

需要可空、可重新赋值或进行地址运算时仍使用原始指针。保存、复制和访问原始指针不需要特殊关键字；程序员承担相同的地址与生命周期责任。
