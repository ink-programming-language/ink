...
> 状态：已确认，议题 34、37 确认异常绑定中的 `as`，Parser 议题 06 确认导入别名中的 `as`，其他用途待定
> 确认日期：2026-08-01
...
Parser 议题 06 还确认 `as` 用于模块导入和成员导入的别名：

```ink
import network.http as net;
from network.http import Client as HttpClient;
```

资源作用域绑定和模式别名等其他场景是否使用 `as`，仍须分别在对应语言特性议题中讨论；异常捕获和导入别名的既有决定不自动推广到其他上下文。
