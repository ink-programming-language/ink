...
Ink 将“允许近似浮点变换”与“承诺不存在 NaN/Infinity”拆成两个独立属性：
...
需要接近 LLVM 完整 fast-math 优化空间时，可以组合：
...
`[fast_math]` 只映射到它明确允许的 fast-math flags，例如重结合、收缩、近似倒数、近似函数和忽略有符号零；不能仅凭它设置会把 NaN 或 Infinity 变成 poison 的 flags。
...
本议题先确认属性语义。以下语法范围在后续议题中决定：

- 仅允许附着到函数；
- 是否允许附着到语句块；
- 是否允许附着到单个表达式；
- 是否允许附着到参数、返回值或用户定义数值类型。
