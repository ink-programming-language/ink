...
| 09 | 语句块与花括号 | 已确认，汇总完整局部声明和语句集合，`var`/`const` 在 block item 起点提交声明，增加 `comptime { ... }` 并接入统一区域控制 | [09-statement-blocks.md](09-statement-blocks.md) |
...
| 18 | 表达式语句与丢弃结果 | 已确认，statement expression 不直接以 `var`/`const` 开始 | [18-expression-statements-and-discarded-results.md](18-expression-statements-and-discarded-results.md) |
...
| 24 | `match` 语句与表达式 | 已确认，statement arm 中的声明必须外包语句块；增加 `comptime match` 并接入统一区域控制；被匹配表达式使用固定括号 | [24-match-statements-and-expressions.md](24-match-statements-and-expressions.md) |
...
| 30 | 普通表达式中的复合类型值 | 已确认，类型尾链通过 `parseExpression(EndSet)` 事务提交，议题 40 补充 class 类型表达式 | [30-compound-type-values-in-expressions.md](30-compound-type-values-in-expressions.md) |
...
| 35 | 聚合初始化表达式 | 已确认，显式类型、命名字段及同名字段简写、无尾随逗号，并可直接作为普通后缀链起点 | [35-aggregate-initialization-expressions.md](35-aggregate-initialization-expressions.md) |
