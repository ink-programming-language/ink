...
| 05 | 源文件与条件导入 | 已确认，顶层声明集合完整定义；声明区支持五种固定控制头括号的 `comptime` 结构并作为议题 32 的 `TopLevelRegion` 实例；顶层绑定接入议题 39 的访问前缀 | [05-source-file-and-conditional-imports.md](05-source-file-and-conditional-imports.md) |
...
| 09 | 语句块与花括号 | 已确认，汇总完整局部声明和语句集合，声明与裸 `match`/`comptime match` 在 block item 起点确定性提交，增加 `comptime { ... }` 并接入统一区域控制 | [09-statement-blocks.md](09-statement-blocks.md) |
...
| 11 | 赋值语句 | 已确认，左侧先排除声明与保留结构起点再按普通表达式解析，复合赋值符号不可拆分，可写性由语义分析检查 | [11-assignment-statements.md](11-assignment-statements.md) |
...
| 18 | 表达式语句与丢弃结果 | 已确认，statement expression 不直接以 `var`/`const`、裸 `match` 或 `comptime match` 开始；丢弃 `MatchExpression` 时整体加括号 | [18-expression-statements-and-discarded-results.md](18-expression-statements-and-discarded-results.md) |
...
| 24 | `match` 语句与表达式 | 已确认，statement entry 的裸 `match`/`comptime match` 不可回滚地提交结构控制，值形式由明确 expression context 选择；statement arm 中的声明必须外包语句块；被匹配表达式使用固定括号 | [24-match-statements-and-expressions.md](24-match-statements-and-expressions.md) |
...
| 32 | 统一的 `comptime` 区域控制 | 已确认，表达式与 module、语句、类、接口、枚举区域共享同一阶段模型；结构化控制头使用固定括号，statement entry 的 `comptime match` 以有限前瞻提交 | [32-unified-comptime-regions.md](32-unified-comptime-regions.md) |
