...
| 09 | 语句块与花括号 | 已确认，汇总完整局部声明和语句集合，增加 `comptime { ... }` 并接入统一区域控制 | [09-statement-blocks.md](09-statement-blocks.md) |
| 10 | `var`、`const` 与元组解构声明 | 已确认，字段同样必须显式写 `var` 或 `const`；module 顶层绑定支持访问前缀，局部绑定不支持；完整字段语法见议题 33 | [10-binding-declarations.md](10-binding-declarations.md) |
| 11 | 赋值语句 | 已确认，左右两侧均按普通表达式解析，左侧可写性由语义分析检查 | [11-assignment-statements.md](11-assignment-statements.md) |
...
| 14 | 基础表达式 | 已确认，议题 40 加入复用普通 class 结构的类型表达式 | [14-primary-expressions.md](14-primary-expressions.md) |
...
| 30 | 普通表达式中的复合类型值 | 已确认，议题 40 补充 class 类型表达式 | [30-compound-type-values-in-expressions.md](30-compound-type-values-in-expressions.md) |
