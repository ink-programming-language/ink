...
其任务启动、暂停、取消和结果复制继续遵守既有异步议题。本节只确定它在表达式文法中的前缀位置和优先级。
...
`comptime if (condition) { ... }`、`comptime for (...) { ... }` 和 `comptime while (condition) { ... }` 是语句上下文中的结构化控制形式，由议题 32 的有限 Token 前瞻分派；其他 `comptime` 操作数继续解析为本节的 `comptime_expression`。
