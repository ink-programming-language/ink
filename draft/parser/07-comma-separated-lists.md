...
> 状态：已确认，议题 29 补充列表展开；2026-08-05 确认枚举分支继续禁止尾随逗号；2026-08-08 同步 `::<...>` 泛型应用拼写
> 确认日期：2026-08-03
...
Parser 先按照源码元素验证逗号，不提前展开元素数量。展开空序列不会使前导、连续或尾随逗号合法。泛型应用的 `::<` 三个字符必须直接相邻；左侧表达式与 `::` 之间可以有 Trivia，规范格式仍为 `name::<T>`。泛型声明仍使用 `<...>`，两者的逗号规则相同。单独的 `(...values)` 和 `(...Types)` 由 `...` 明确选择元组结构，不需要单元素尾随逗号；`(...values,)` 和 `(...Types,)` 仍然非法。
...
Ink 的普通逗号列表由一个元素和零个或多个“逗号加元素”组成，不接受尾随逗号。多行布局和语义阶段的列表展开不改变源码逗号规则。`(element,)` 的逗号用于单元素元组消歧；单独的 `(...expression)` 由展开标记完成消歧。这些都由各自结构的专用产生式要求，不是普通列表的可选尾随格式。
