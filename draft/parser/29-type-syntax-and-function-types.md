...
它用于绑定类型标注、参数和字段类型、函数结果、泛型形参约束以及后续声明议题明确引用类型的位置。Parser 不为这些位置分别维护互不一致的指针、数组或函数类型子文法。
...
`()`、`(T,)` 和多元素元组保持明确区分并共享中性圆括号逗号列表 CST，类型序列使用列表级展开；泛型展开的操作数使用 `generic_argument_expression` 定界。函数类型写作 `[async] func(types) [-> type]`，可以作为普通表达式中的类型值；省略结果等价于 `void`，`async` 属于函数类型。函数、元组、泛型和普通实参列表可以在各自确认的位置使用前置展开，但 `...` 不成为普通表达式或类型后缀。
