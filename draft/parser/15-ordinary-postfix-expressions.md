...
完整后缀表达式分为封闭的直接函数类型表达式，以及能够继续附加后缀的基础表达式链：
...
```ink
func() -> T*   // 返回 T*，星号属于返回类型
(func() -> T)* // 星号应用于整个函数类型
(func())()     // 对加括号后的函数类型值应用调用后缀
```
...
`->` 属于议题 02 的全语言复合符号集合。普通表达式看到相邻 `->` 时不能先把 `-` 消费为加减运算符；当前位置若不允许指针成员后缀或其他 `->` 语法角色，就应保留完整序列用于外层判断或产生诊断。
...
Ink v0 不定义 `?.`、`?->` 或其他可选链后缀。原始指针需要显式检查 `null`，`Optional::<T>` 等枚举值使用显式判别和对应标准库 API 处理：
...
议题 16 已经定义以复合终端 `::<` 明确引导的显式泛型实例化 `::<...>`。它与本议题的普通后缀共同构成完整后缀链，并且不与比较 `<` 竞争。构成 `::<` 的三个 Symbol Token 必须直接相邻；左侧表达式与整个 `::<` 终端之间可以有普通 Trivia，但格式化器的规范输出应紧贴为 `name::<...>`。
