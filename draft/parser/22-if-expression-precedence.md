...
```text
if (first) then a else (if (second) then b else c)
```
...
关键字配对由递归产生式确定：内层 `if` 消费自己的 `then` 和 `else`，之后的 `else c` 与外层 `if` 配对。为了提高阅读清晰度可以加括号，但语法不强制。
...
if (condition) then a else b        → IfExpression
```

当第二种形式作为表达式语句使用时，仍须按照议题 18 在末尾写分号：

```ink
if (condition) then perform_a() else perform_b();
```

花括号与 `then` 是稳定的语法分界，不需要类型信息参与消歧。
