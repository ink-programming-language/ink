...
本议题不改变以花括号自行结束的专用语句。`if`、`match` 等结构在语句位置具有专门语句产生式时，继续由其自身规则决定是否需要结尾分号。

如果某个结构化表达式确实通过 `expression_statement` 使用，其外层表达式语句仍必须以 `;` 结束。具体的语句与表达式消歧由相应控制结构议题定义。
...
Ink 允许任何语法正确的表达式形成以分号结束的表达式语句，并允许直接丢弃非 `void` 结果。表达式仍完整求值，未消费临时结果在分号处析构。v0 不增加 `discard` 或 `_ =` 特殊语法；明显无效果表达式和专用 must-use 检查留给 diagnostics 与后续语义议题。
