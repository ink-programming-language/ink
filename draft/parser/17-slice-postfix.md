...
采用冒号切片不会为普通表达式引入 `..`。Ink v0 当前没有由本议题建立的内建范围表达式、范围值或相关优先级。

未来如果需要通用 Range 类型或范围循环，应单独设计其值语义、包含性、整数类型和运算符文法，不能从切片冒号隐式推导。
...
Ink 使用 `[:]`、`[start:]`、`[:end]` 和 `[start:end]` 四种左闭右开切片后缀，不使用 `..`。切片只适用于数组和安全切片，继承元素可写性，边界违反 `start <= end <= length` 时 trap；原始指针不能借此建立安全切片。v0 不提供步长、包含式结束边界或通用范围运算符。
