...
类型语法和议题 16 的泛型实参后缀可以逐个消费泛型结束符：

```ink
Array<Array<i32>>
```

这里最后两个 `>` 始终是两个真实 Token。普通表达式 Parser 可以把它们组织为右移运算符，类型或泛型后缀 Parser 可以把它们分别用作两层泛型的结束符。Tokenizer 不需要重新分词，Parser 也不需要拆分 Token。
