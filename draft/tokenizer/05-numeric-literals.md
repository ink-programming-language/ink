...
数值字面量内部的每个下划线必须严格位于同一数字组成部分的两个合法数字之间。以下形式非法：
...
`_100` 不进入数值字面量扫描；它符合标识符规则，产生 `Identifier("_100")` 且不产生词法诊断。
...
`.5` 和 `1.` 不构成错误数字 Token，也不产生词法诊断。它们分别扫描为 `Symbol(".")` 与 `IntegerLiteral("5")`，以及 `IntegerLiteral("1")` 与 `Symbol(".")`。这些 Token 在具体语法位置是否合法，以及是否应建议写成 `0.5` 或 `1.0`，由 parser 诊断。
