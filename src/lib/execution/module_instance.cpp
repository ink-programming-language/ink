...
namespace ink::execution
{
  ...
  {
  public:
    Impl(const ModuleLoader &Owner, ir::ModuleId Id, std::shared_ptr<const ir::Module> Definition, core::TargetContext Target)
        : Owner(&Owner),
          Id(Id),
    ...
    std::vector<ir::ModuleId> ActiveDependencies;
  ...
  ModuleInstance::ModuleInstance(const ModuleLoader &Owner, ir::ModuleId Id, std::shared_ptr<const ir::Module> Definition, core::TargetContext Target)
      : Implementation(std::make_unique<Impl>(Owner, Id, std::move(Definition), Target))
  {
  }

  ModuleInstance::~ModuleInstance() = default;

  ir::ModuleId ModuleInstance::id() const noexcept
  {
  ...
  std::vector<ir::ModuleId> ModuleInstance::activeDependencies() const
  {
    const std::lock_guard<std::mutex> Lock(Implementation->Mutex);
  ...
  {
    const std::lock_guard<std::mutex> Lock(Implementation->Mutex);
    return Implementation->State == ModuleState::Initializing;
  }
  ...
  ModuleLoadError ModuleInstance::completeLoading(ModuleLoadError Error) noexcept
  {
    ...
    {
      if (!Implementation->Definition->Globals[GlobalIndex].Mutable)
      {
  ...
  bool ModuleInstance::addActiveDependency(ir::ModuleId Dependency) noexcept
  {
    const std::lock_guard<std::mutex> Lock(Implementation->Mutex);
    if (Implementation->State != ModuleState::Initializing)
    {
      return false;
    }
