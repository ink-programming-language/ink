...
var NUM: dynamic = cpp_expression("#inc");

class Edge
{
  func Edge(arg_to: dynamic, arg_capacity: dynamic, arg_rev_index: dynamic) -> dynamic
  {
      to = arg_to;
  ...
  var rev_index: dynamic = cpp_uninitialized();
}
...
var G: dynamic = cpp_array(NUM);

var CAP1: dynamic = cpp_array(NUM);

var dist: dynamic = cpp_array(NUM);

var cheked_index: dynamic = cpp_array(NUM);

var can_visit: dynamic = cpp_array(NUM);

func add_edge(from_cpp: dynamic, to: dynamic, capacity: dynamic) -> dynamic
{
...
func bfs(source: dynamic) -> dynamic
{
  {
    var i: dynamic = 0;
    while ((i < V))
  ...
  while ((!Q.empty()))
  {
    var node_id: dynamic = Q.front();
    Q.pop();
    {
      var i: dynamic = 0;
      while ((i < G[node_id].size()))
      {
        var e: dynamic = G[node_id][i];
...
func dfs(node_id: dynamic, sink: dynamic, flow: dynamic) -> dynamic
{
  if ((node_id == sink))
  {
    return flow;
  ...
  {
    var i: dynamic = cheked_index[node_id];
    while ((i < G[node_id].size()))
    {
      var e: dynamic = G[node_id][i];
      if (((e.capacity > 0) && (dist[node_id] < dist[e.to])))
      {
        var tmp_flow: dynamic = dfs(e.to, sink, min(flow, e.capacity));
        if ((tmp_flow > 0))
        {
          e.capacity -= tmp_flow;
...
func recursive(node_id: dynamic) -> dynamic
{
...
func max_flow(source: dynamic, sink: dynamic) -> dynamic
{
  var flow: dynamic = 0;
...
func main() -> dynamic
{
  ...
  var source: dynamic = 0;
  var sink: dynamic = (V - 1);
  var flow: dynamic = max_flow(source, sink);
  if ((flow >= 10002))
  {
    printf("-1\n");
    return 0;
